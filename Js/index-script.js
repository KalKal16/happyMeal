...
document.addEventListener("DOMContentLoaded", function() {
    const recettesContainer = document.getElementById("recettesContainer");
    const afficherToutesRecettesBtn = document.getElementById("afficherToutesRecettesBtn");
    ...
    function afficherRecetteCard(recette) {
        const card = document.createElement('div');
        card.classList.add('col-md-6', 'mb-4');
        card.innerHTML = `
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-text">Temps de préparation: ${recette.temps_preparation}</p>
                    <p class="card-text">Catégorie: ${recette.categorie}</p>
                    <button class="btn btn-primary voir-recette" data-bs-toggle="modal" data-bs-target="#recetteModal" data-id="${recette.id}">Voir la recette</button>
                </div>
            </div>
        `;
        recettesContainer.appendChild(card);
    ...
    async function afficherToutesRecettes() {
        const recettes = await chargerRecettes();
        recettesContainer.innerHTML = "";
        recettes.forEach(recette => {
            afficherRecetteCard(recette);
        });
    }

    afficherToutesRecettesBtn.addEventListener("click", afficherToutesRecettes);
});
